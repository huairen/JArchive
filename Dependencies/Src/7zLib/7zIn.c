...
int SzFolder_FindBindPairForInStream(const CSzFolder *p, UInt32 inStreamIndex)
{
  UInt32 i;
...
int SzFolder_FindBindPairForOutStream(const CSzFolder *p, UInt32 outStreamIndex)
{
  UInt32 i;
  for (i = 0; i < p->NumBindPairs; i++)
...
int SzFolder_FindPackStreamsIndex(const CSzFolder *p, UInt32 inStreamIndex)
{
	UInt32 i;
	for (i = 0; i < p->NumPackStreams; i++)
		if (p->PackStreams[i] == inStreamIndex)
			return i;
	return -1;
}

void SzFolder_FindInStream(const CSzFolder *p, UInt32 streamIndex, UInt32 *coderIndex, UInt32 *coderStreamIndex)
{
	UInt32 i;
	for (i = 0; i < p->NumCoders; i++)
	{
		UInt32 curSize = p->Coders[i].NumInStreams;
...
void SzFolder_FindOutStream(const CSzFolder *p, UInt32 streamIndex, UInt32 *coderIndex, UInt32 *coderStreamIndex)
{
	UInt32 i;
	for (i = 0; i < p->NumCoders; i++)
	{
		UInt32 curSize = p->Coders[i].NumOutStreams;
		if (streamIndex < curSize)
		{
			*coderIndex = i;
			*coderStreamIndex = streamIndex;
...
#define MY_ALLOC(T, p, readSize, alloc) { if ((readSize) == 0) p = 0; else \
  if ((p = (T *)IAlloc_Alloc(alloc, (readSize) * sizeof(T))) == 0) return SZ_ERROR_MEM; }
...
SRes SzArEx_Open(CSzArEx *p, ISeekInStream *inStream, ISzAlloc *allocMain, ISzAlloc *allocTemp)
{
