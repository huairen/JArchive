...
int MY_CDECL main(int numargs, char *args[])
{
	...
	res = SzArEx_Decompress(args[1],"E:\\test7z\\",NULL);
...
SRes SzArEx_Decompress( const char* pFilename, const char* pOutputDir, ICompressProgress* progress )
{
	...
	if (InFile_Open(&archiveStream.file, pFilename))
	...
	tempSize = MultiByteToWideChar(CP_ACP, 0, pOutputDir, -1, NULL, 0);
	tempPath = (UInt16*)IAlloc_Alloc(&allocImp,tempSize*sizeof(UInt16));
	MultiByteToWideChar(CP_ACP, 0, pOutputDir, -1, tempPath, tempSize); 

	if(res == SZ_OK)
		res = SzArEx_Extract(&db, &archiveStream.s, tempPath, progress, &allocTempImp);

	IAlloc_Free(&allocImp,tempPath);
